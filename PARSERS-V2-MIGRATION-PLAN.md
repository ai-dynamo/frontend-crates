...
`v2` means frontend-crate-owned parser code, Python binding code, stream fixtures, batch-on-stream fixtures, and conformance renderer behavior. During the bridge period, v2 lives in `parsers_v2/`, `parsers_v2-py/`, `conformance/toolcalling/fixtures-stream-v2/`, `conformance/toolcalling/fixtures-batch-on-stream-v2/`, `conformance/utils/generate_conformance_table.py`, and `conformance/utils/conformance_table.html.j2`.
...
| `conformance/utils/generate_conformance_table.py` and `conformance/utils/conformance_table.html.j2` | v2 frontend-crate-owned | New conformance table renderer. |
...
| `conformance/utils/render_table.sh` | Renders `conformance/CONFORMANCE.html` with the v2 conformance generator. |
...
| `conformance/utils/capture_driver.py` | Host orchestrator: `--mode stream` batch-captures non-Harmony stream fixtures, `--mode batch-on-stream` rewrites the overlays, `--mode merge` builds `harmony_batch_stream.json`. |
| `conformance/utils/harmony_batch_stream.json` | Recorded batch-on-stream comparison data consumed by the v2 table. |
| `conformance/utils/generate_conformance_table.py` | frontend-crate-owned conformance renderer; staged into `tests/parity/` at render time. |
| `conformance/utils/conformance_table.html.j2` | frontend-crate-owned conformance HTML template; staged into `tests/parity/` at render time. |
