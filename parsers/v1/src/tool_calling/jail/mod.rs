...
mod tests {
    ...
    /// bundled shape reported on GLM-5.3/5.3-Flash: one delta with `content`
    /// (leftover prose) AND `tool_calls[0].function.name` both set.
    #[allow(deprecated)]
    ...
    /// Reproduces a reported GLM streaming shape: a delta that bundles
