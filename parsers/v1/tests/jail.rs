...
mod tests {
    ...
    // Muse Glimmer jail inputs mirror what the muse_glimmer reasoning parser
    // forwards: plain content plus tool channels normalized to
    // `<|start|>assistant to=<fn><|message|>ATEM<|eom|>`.
    #[tokio::test]
    async fn test_muse_glimmer_single_call_with_surrounding_content() {
        let chunks = vec![
            create_mock_response_chunk("Let me check. ".to_string(), 0),
            create_mock_response_chunk(
                "<|start|>assistant to=get_weather<|message|>".to_string(),
                0,
            ),
            create_mock_response_chunk(
            ...
            create_mock_response_chunk(
                "</atem:invoke>\n</atem:function_calls><|eom|>".to_string(),
                0,
            ),
        ...
        let tool_chunk = results
            .iter()
            .find(|result| test_utils::has_tool_call(result))
            .expect("framed ATEM channel should emit a tool call");
        test_utils::assert_tool_call(
            tool_chunk,
            "get_weather",
    ...
    #[tokio::test]
    async fn test_muse_glimmer_parallel_calls_in_chained_channels() {
        let chunks = vec![
            create_mock_response_chunk(
                concat!(
        ...
        let tool_chunks: Vec<_> = results
            .iter()
            .filter(|result| test_utils::has_tool_call(result))
            .collect();
        assert_eq!(tool_chunks.len(), 2, "each chained channel emits its call");
        test_utils::assert_tool_call(
            tool_chunks[0],
            "get_weather",
            serde_json::json!({"city": "Paris"}),
    ...
    #[tokio::test]
    async fn test_muse_glimmer_chained_channels_in_one_delta_emit_before_terminal() {
        // Interval batching can deliver both <|eom|>-chained channels in ONE
        // backend delta. Both calls must come out of that delta's completion
        // pass; a trailing buffered call that only surfaces at teardown would
        // emit after the terminal/usage chunks.
        let chunks = vec![
            create_mock_response_chunk(
                concat!(
                    "<|start|>assistant to=get_weather<|message|><atem:function_calls>\n",
        ...
        let tool_indices: Vec<usize> = results
            .iter()
            .enumerate()
            .filter(|(_, r)| test_utils::has_tool_call(r))
            .map(|(i, _)| i)
            .collect();
        let finish_indices: Vec<usize> = results
            .iter()
            .enumerate()
            .filter(|(_, r)| {
                r.data
                    .as_ref()
                    .is_some_and(|d| d.choices.iter().any(|c| c.finish_reason.is_some()))
            })
            .map(|(i, _)| i)
            .collect();
        let calls: Vec<(String, serde_json::Value)> = results
            .iter()
            ...
            .map(|tc| {
                ...
                (
                    f.name.clone().expect("name present"),
                    serde_json::from_str(f.arguments.as_deref().expect("arguments present"))
        ...
        assert_eq!(
            ...
            "both chained calls must be emitted, in order"
        );
        let first_finish = finish_indices.first().copied().unwrap_or(usize::MAX);
        assert!(
            tool_indices.iter().all(|i| *i <= first_finish),
            "tool calls {tool_indices:?} must not follow the first finish chunk {finish_indices:?}"
        );
        assert_eq!(test_utils::reconstruct_content(&results), "");
    }

    #[tokio::test]
    async fn test_muse_glimmer_narration_between_channels_in_one_delta() {
        // One interval-batched delta: call A, unwrapped narration, call B.
        // Both calls and the narration must all surface before the terminal.
        let chunks = vec![
            create_mock_response_chunk(
                concat!(
                    "<|start|>assistant to=get_weather<|message|><atem:function_calls>\n",
        ...
        assert_eq!(
            ...
            "both calls must surface"
        );
        assert_eq!(test_utils::reconstruct_content(&results), " Now the time. ");
        let finish_idx = results
            .iter()
            .position(|r| {
                r.data
                    .as_ref()
            ...
            .unwrap_or(usize::MAX);
        let last_call_idx = results
            .iter()
            .rposition(test_utils::has_tool_call)
            .expect("calls present");
        assert!(
            last_call_idx <= finish_idx,
            "no tool call may follow the finish chunk (last call {last_call_idx}, finish {finish_idx})"
        );
    }

    #[tokio::test]
    async fn test_muse_glimmer_whitespace_separated_channels_in_one_delta() {
        // Same as the narration variant with a whitespace-only gap between the
        // two complete channels in one delta.
        let chunks = vec![
            create_mock_response_chunk(
                concat!(
                    "<|start|>assistant to=get_weather<|message|><atem:function_calls>\n",
        ...
        let calls: Vec<String> = results
            ...
            .filter_map(|tc| tc.function.as_ref().and_then(|f| f.name.clone()))
            .collect();
        assert_eq!(calls, vec!["get_weather", "get_time"]);
        let finish_idx = results
            .iter()
            .position(|r| {
                r.data
        ...
        assert!(
            last_call_idx <= finish_idx,
            "call after finish: {last_call_idx} vs {finish_idx}"
        );
    }

    #[tokio::test]
    async fn test_muse_glimmer_prefixed_channels_then_empty_chunk_order() {
        // Adversarial drain shape: prose prefix, two complete channels split
        // by whitespace in ONE delta, then a content-less chunk before the
        // terminal. Both calls must still precede the finish chunk.
        let chunks = vec![
            create_mock_response_chunk(
                concat!(
                    "Checking. ",
            ...
            create_mock_response_chunk(String::new(), 0),
            create_final_response_chunk(0),
        ];
        ...
        assert_eq!(test_utils::reconstruct_content(&results), "Checking.  ");
    }

    #[tokio::test]
    async fn test_muse_glimmer_missing_eom_recovers_at_finalize() {
        let chunks = vec![
            create_mock_response_chunk(
                concat!(
                    "<|start|>assistant to=get_weather<|message|><atem:function_calls>\n",
        ...
        let tool_chunk = results
            .iter()
            .find(|result| test_utils::has_tool_call(result))
            .expect("complete invoke without <|eom|> should recover when the stream finalizes");
        test_utils::assert_tool_call(
            tool_chunk,
            "get_weather",
            serde_json::json!({"city": "Paris"}),
    ...
    #[tokio::test]
    async fn test_muse_glimmer_quoted_atem_in_content_is_not_a_call() {
        // ATEM markup without channel framing is quoted text (for example an
        // unwrapped `to=user` body explaining the format), never a live call.
        let chunks = vec![
            create_mock_response_chunk(
                "Example: <atem:function_calls><atem:invoke ".to_string(),
                0,
            ),
            create_mock_response_chunk(
                "name=\"demo\"></atem:invoke></atem:function_calls>".to_string(),
                0,
            ),
            create_final_response_chunk(0),
        ];
        ...
        assert_eq!(
            test_utils::reconstruct_content(&results),
            "Example: <atem:function_calls><atem:invoke name=\"demo\"></atem:invoke></atem:function_calls>"
        );
    }

    #[tokio::test]
    async fn test_muse_glimmer_every_single_split_boundary_emits_the_same_call() {
        let input = concat!(
            "<|start|>assistant to=get_weather<|message|><atem:function_calls>\n",
            "<atem:invoke name=\"get_weather\">\n",
            "<atem:parameter name=\"city\">Paris</atem:parameter>\n",
