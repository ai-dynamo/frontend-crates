...
///
/// Owned (upstream bug): async-openai serializes `None` fields as explicit
/// `null`, but the spec marks every field optional and non-nullable, so
/// absent fields must be omitted (OpenAI emits `"audio_tokens": 0`, never
/// `null`). Field set mirrors upstream `PromptTokensDetails`.
#[derive(Debug, Deserialize, Serialize, Clone, PartialEq, Default)]
...
///
/// Owned for the same reason as [`PromptTokensDetails`]: every field is
/// optional and non-nullable in the spec, so `None` must be omitted.
#[derive(Debug, Deserialize, Serialize, Clone, PartialEq, Default)]
pub struct CompletionTokensDetails {
    ...
    /// that did not appear in the completion (still billed and counted in
...
///
/// Owned so `usage` uses our [`PromptTokensDetails`] /
/// [`CompletionTokensDetails`] (upstream serializes their absent fields as
/// `null`). Field set mirrors upstream `CompletionUsage`.
#[derive(Debug, Deserialize, Serialize, Clone, PartialEq, Default)]
...
mod tests {
    ...
    /// Spec shape: optional fields are omitted when absent, while
    /// required-and-nullable fields (`content`, `refusal`, `finish_reason`,
    /// `logprobs`) stay present.
    #[test]
    #[allow(deprecated)]
    ...
    fn stream_response_omits_absent_optional_fields() {
        ...
        // Required-and-nullable in the stream choice: present as null.
        let choice = &json["choices"][0];
        ...
        for absent in [
            ...
            assert!(
                ...
                "delta.{absent} should be omitted"
            );
        }
    }
    ...
    /// Spec: every usage-details field is optional and non-nullable, so
    ...
    fn usage_details_omit_absent_fields() {
        ...
        assert!(
            json["prompt_tokens_details"].get("audio_tokens").is_none(),
        ...
        assert_eq!(json["reasoning_tokens"], 5);
        for absent in [
