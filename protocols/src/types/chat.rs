...
/// System message with model-specific dynamic tool metadata support.
///
/// Some OpenAI-compatible chat templates accept a system message that carries
/// dynamic tool declarations without ordinary text content. Keep the extra
/// metadata typed as JSON so downstream renderers can decide whether it applies.
...
mod tests {
    ...
    #[test]
    fn assistant_message_accepts_reasoning_alias() {
        let message: ChatCompletionRequestMessage = serde_json::from_value(serde_json::json!({
            "role": "assistant",
            "reasoning": "synthetic prior reasoning",
        ...
        assert!(matches!(
            ...
            Some(ReasoningContent::Text(ref text)) if text == "synthetic prior reasoning"
        ));
    }
    ...
    fn system_message_accepts_tools_without_content() {
        ...
        assert_eq!(system.tools.as_ref().unwrap()[0]["name"], "lookup");
    }

    #[test]
    fn assistant_message_accepts_partial_prefill_marker() {
        let message: ChatCompletionRequestMessage = serde_json::from_value(serde_json::json!({
            "role": "assistant",
