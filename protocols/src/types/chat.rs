...
pub struct ChatCompletionRequestAssistantMessage {
    ...
    /// Moonshot partial mode: when true on the final message, the model
    /// continues this content instead of opening a new assistant turn
    /// (equivalent to vLLM `continue_final_message`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub partial: Option<bool>,
}

/// System message with Moonshot dynamic tool loading.
///
/// Extends upstream `ChatCompletionRequestSystemMessage` with:
/// - `tools`: tool definitions injected mid-conversation (Kimi K3)
/// - `content` may be omitted on the wire when the message only carries `tools`
#[derive(Debug, Serialize, Deserialize, Default, Clone, Builder, PartialEq)]
...
/// (with reasoning_content), `ChatCompletionRequestSystemMessage` (with tools),
/// and `ChatCompletionRequestUserMessage` (which references our extended
/// content parts with video/audio).
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq)]
#[serde(tag = "role")]
#[serde(rename_all = "lowercase")]
