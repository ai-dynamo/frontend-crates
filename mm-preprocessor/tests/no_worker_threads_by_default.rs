...
//! Without the `parallel` feature this crate must not own worker threads: a
//! serving engine supplies concurrency across requests and pins its own
...
//! `par` is refactored. Runs only in the default (rayon-less) build; under
//! `--features parallel` the pools are expected.

#![cfg(all(not(feature = "parallel"), target_os = "linux"))]
...
fn processing_a_request_spawns_no_worker_threads() {
    ...
    assert!(
        spawned.is_empty(),
        "default build spawned crate-owned worker threads: {spawned:?}"
    );
    assert_eq!(
        after.len(),
        before,
        "default build changed the process thread count: {after:?}"
    );
}
