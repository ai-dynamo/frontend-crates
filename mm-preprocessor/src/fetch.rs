...
//! the proxy env vars like Python), `file://` / absolute path, `data:` URL,
//! else bare base64.
...
/// Knobs of the network stage; [`Default`] matches the Python path's defaults.
#[derive(Clone, Debug)]
#[non_exhaustive]
pub struct FetchOptions {
    /// Per-source HTTP GET timeout (Python: `REQUEST_TIMEOUT`, default 3 s).
...
pub fn fetch_bytes_budgeted_with(
    ...
    if src.starts_with("http://") || src.starts_with("https://") {
        return http_get(src, budget, opts);
    }
...
fn http_get(url: &str, budget: &ByteBudget, opts: &FetchOptions) -> Result<Vec<u8>, String> {
