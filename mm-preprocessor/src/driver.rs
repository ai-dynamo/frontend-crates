...
pub struct OutputItem {
    ...
    /// source bytes — the same identity role as the Python path's
    /// `hash_feature`, but a different algorithm, so hashes are consistent
    /// within this pipeline and never comparable across the two paths.
    pub hash: u64,
}
...
fn resolve<'a>(
    ...
    match source {
        #[cfg(feature = "fetch")]
        ImageSource::String(source) => Ok(fetch::fetch_bytes_budgeted_with(
            source,
        ...
        ImageSource::String(_) => {
            let _ = opts;
            Err("string media sources require the `fetch` feature".into())
        }
...
pub fn process_with(
    ...
    let processed: Vec<(ProcessedItem, u64)> =
        par::try_map(&fetched, |bytes| -> Result<(ProcessedItem, u64), String> {
            let hash = crate::content_hash_u64(bytes);
            // Inputs PIL accepts but decode_rgb refuses (e.g. 16-bit PNG)
            // error here and reject the request.
            let (rgb, height, width) = crate::image::decode::decode_rgb(bytes)?;
...
mod tests {
    ...
    fn fetches_url_and_file_sources() {
        let png = png(8, 8);
        let dir = std::env::temp_dir().join(format!("dynamo-mm-driver-{}", std::process::id()));
