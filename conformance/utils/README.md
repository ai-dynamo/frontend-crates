...
### GLM-family capture and JSON gate

GLM-5.3 uses the existing `glm47` fixture family because its published chat template emits the same `<tool_call>`, `<arg_key>`, and `<arg_value>` grammar. This family ID is shared with GLM-4.7/5.1; it does not claim that the committed corpus was generated by a live GLM-5.3 inference run. Do not create a separate `glm53` fixture directory unless the model's output grammar differs. The `refresh_dynamo_captures.py` command records Dynamo v2 stream and batch-on-stream results under an attributable label, and the `package_fixtures.py` step is required before the renderer can read them.

```bash
# Capture Dynamo v2 stream and batch-on-stream outputs for every registered v2 family.
# The current refresh script has no family filter; use the full-corpus result when
# changing the parser crate, then package the complete capture tree.
python3 conformance/utils/src/refresh_dynamo_captures.py stream batch-on-stream --label 0.6.0+<ticket-or-sha>

# Rebuild the fixture shards and manifest. Do not use --dry-run for the capture being validated.
python3 conformance/utils/src/package_fixtures.py
python3 conformance/utils/src/extract_fixtures.py --full-refresh

# Render both artifacts from the same extracted snapshot.
conformance/utils/render_table_v2.sh

# The status command rerenders, writes CONFORMANCE_v2.json, lists every empty/red cell,
# and exits nonzero until the selected family/tab is clear. The stream-v2 tab is the
# current Dynamo v2 gate for this family; batch-on-stream is covered by the fixture test.
conformance/utils/check.sh status --model glm47 --tab tab-toolcalling-streamv2
```

The capture is passing only when the scoped status command exits `0` and reports zero empty and zero red cells, the Dynamo stream-on-batch fixture test passes, and the parser tests pass. The legacy batch tab can still show historical v1 gaps; do not use that tab as the Dynamo v2 GLM gate. A generated JSON file or a successful render alone does not prove the captured GLM data matches the parser.
