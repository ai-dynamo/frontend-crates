...
_VISIBLE_CONFORMANCE_REPLACEMENTS = (
    ...
    ("Dynamo Parser Parity Table", "Dynamo Parser Conformance Table"),
    ("Dynamo Reasoning Parser - Parity Table", "Dynamo Reasoning Parser - Conformance Table"),
    ("Dynamo Tool Calling Parser - Parity Table", "Dynamo Tool Calling Parser - Conformance Table"),
    ("Parity Table", "Conformance Table"),
    ("parity table", "conformance table"),
    ("tests/parity/README.md", "Dynamo parser fixture README"),
)
...
def render_html(modes: list[str], family_filter: str | None = None) -> str:
    ...
    title = (
        f"Dynamo {family_filter} Tool Calling Parser - Conformance Table"
        if family_filter
...
def render_combined_html(
    ...
    html = (
        ...
        .get_template("conformance_table_v2.html.j2")
        .render(
            title="Dynamo Parser Conformance Table",
            stamp=stamp,
            sha=sha,
            short_sha=sha[:12] if sha else "",
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate Dynamo conformance tables.",
    )
    parser.add_argument(
        "stage",
    ...
    if args.stage == "all":
        stage_parser = argparse.ArgumentParser(
            description="Generate the combined Dynamo parser conformance HTML page.",
        )
