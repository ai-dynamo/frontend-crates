...
def _write_corpus(tmp_path, chunks, model_text=MODEL_TEXT, ref="TOOLCALLING.batch.1"):
    ...
    lines = ["family: fam", "mode: streamv1", "cases:", "  TOOLCALLING.streamv1.1:",
    ...
    (stream / "TOOLCALLING.streamv1.1.yaml").write_text("\n".join(lines) + "\n")
    return stream, batch


def _joined(stream_dir):
    doc = yaml.safe_load((stream_dir / "TOOLCALLING.streamv1.1.yaml").read_text())
    case = doc["cases"]["TOOLCALLING.streamv1.1"]
...
def test_restores_folded_newlines_across_chunk_boundaries(tmp_path):
    ...
    rsi.repair_file(stream / "TOOLCALLING.streamv1.1.yaml",
...
def test_chunk_lengths_are_preserved(tmp_path):
    ...
    rsi.repair_file(stream / "TOOLCALLING.streamv1.1.yaml",
                    rsi._batch_model_text(batch.parent), apply=True)
    doc = yaml.safe_load((stream / "TOOLCALLING.streamv1.1.yaml").read_text())
    got = [len(ch["delta_text"]) for ch in doc["cases"]["TOOLCALLING.streamv1.1"]["chunks"]]
...
def test_leaves_an_uncorrupted_case_untouched(tmp_path):
    ...
    stream, batch = _write_corpus(tmp_path, chunks)
    before = (stream / "TOOLCALLING.streamv1.1.yaml").read_text()
    cases, lines = rsi.repair_file(stream / "TOOLCALLING.streamv1.1.yaml",
                                   rsi._batch_model_text(batch.parent), apply=True)
    assert (cases, lines) == (0, 0)
    assert (stream / "TOOLCALLING.streamv1.1.yaml").read_text() == before
