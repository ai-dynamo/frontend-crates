...
- **`7-4`** (`qwen_string_null`) Qwen3 and GLM declare `city: string` (non-nullable) and preserve bare parameter text `null` as the string `"null"`. The other six families declare `city: string | null` and use native null syntax, with JSON `null` as the expected value. These variants intentionally use different schemas and expected types, both explained in the popup. The Qwen3/GLM variant is also covered in `TOOLCALLING.streamv1.7-1` and `TOOLCALLING.xml.2`.
...
- DeepSeek V4.1 uses the ordered Unified contract for native DSML calls, reasoning interleaving, guided JSON, and prefilled states. The current corpus emits 82 of the 95 taxonomy cases for this family.
...
- The 13 omitted cases are `kimi-1` through `kimi-8`, which require Kimi K3 XTML syntax; `gemma-1` through `gemma-2`, which require Gemma 4 guided call-prefix syntax; `glm5-1`, which requires GLM's argument-marker grammar; `muse-1`, whose non-Muse variant duplicates `35-1`; and the Qwen3-only nullable case `qwen-1`. The `muse-1` duplicate does not imply that quoted or malformed model output cannot occur.
