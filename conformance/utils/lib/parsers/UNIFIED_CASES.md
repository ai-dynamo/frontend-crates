...
New case IDs always use a numeric suffix: `<num>-<num>` for numeric groups or `<letters/num>-<num>` for named groups such as `k3-1`. Existing legacy lettered IDs remain as historical fixture identifiers; do not create new ones. The scenario slug is shown in parentheses. **Groups 1–9 mirror the tool-calling STREAM taxonomy** (`TOOLCALLING.streamv2.N`) as reasoning-free unified cases — this surface subsumes STREAM. **Group 10** is the reasoning axis (`REASONING.*`). **Group 11 is UNIQUE to unified**: reasoning↔tool ORDER that neither STREAM (no reasoning) nor REASONING (no ordered tool events) can express. **Group 12** is adversarial nesting — a marker of one channel inside another (P7). **Group k3** covers Kimi K3's XTML-specific typed arguments, raw JSON blocks, spacing variants, message termination, elided reasoning closes, recovery, and guided wrappers. **Groups 30+ are REQUEST-SCOPED modes** — what the serving layer told the parser about this request, rather than what the model emitted; they use PAIRED TENS, `X0` for a mode's happy path and `X1` for its malformed counterpart, so a recovery case never sorts next to the baseline it contrasts with. The live per-family and total case counts come from `test_unified_case_counts_match_the_generator`; do not maintain a numeric total in prose.

### Group 1 — TC Single call
- **`1.a`** (`tool_only`) One tool call, no reasoning, no surrounding text. The tool suite's baseline.
- **`1-2`** (`kimi_k2_optional_prefix_name_overlap`) A Kimi K2 native identifier whose full name is `functions.`. The optional namespace prefix overlaps the accepted name, so delivering the colon and index in separate chunks must preserve the same call as whole-input parsing.
...
- **`7-3`** (`deepseek_v41_mixed_control_text_in_string`) DeepSeek V4.1 string data combines reasoning markers, calls and invoke closers, an entity, quotes, and a backslash. This retains the earlier V4.1 stress input separately from the shared `7.b` command example.
...
- DeepSeek V4.1 uses the ordered Unified contract for native DSML calls, reasoning interleaving, guided JSON, and prefilled states. The current corpus emits 82 of the 106 taxonomy cases for this family.
- Every taxonomy scenario declared for DeepSeek V4.1 is generated, except `40.a`, `40.c`, and `40.d`: each repeats an existing native Reasoning-to-call/text boundary with only different literal prose. The applicable cases include `7-3`, `30.m`, and `30-14`; `31-1` through `31-25`, `31-27`, `31-28`, `31-33`, and `31-40`; the marker-discriminating Response rows `50.d` and `50-1`; and `40.b` plus `41.a-b`.
- The 24 intentional not-applicable cases are `1-2`, which requires Kimi K2's optional namespace prefix; `k3-1` through `k3-8`, which require Kimi K3 XTML syntax; `g4-1` through `g4-2`, which require Gemma 4 guided call-prefix syntax; `31-26`, which distinguishes Muse's tool-recipient header from its reasoning header; `31-31` and `31-32`, which exercise Qwen's non-ASCII function-header recovery; `31-34` through `31-39`, which are explicitly scoped to Qwen, Muse, DeepSeek V4, or Gemma recovery; `50-2`, which exercises a Muse invoke closer inside an unfinished quoted header; and the three redundant DeepSeek V4.1 rows above. These are grammar-specific cases, explicitly scoped regressions, or literal-only duplicates, not missing coverage.
- `30.m` retains the historical bare header with no tool name. `30-14` adds a tool name before the intentionally missing header terminator; the later `>` in its JSON string argument must remain payload data. Distinct IDs keep historical results attached to the input that was actually captured. `31-8` uses an unfinished DSML invoke header inside reasoning rather than a completed calls-block opener. Marker-free prefilled-Response rows are omitted because their default-state siblings already cover native and guided valid, multi-call, truncated, and malformed inputs; `50.d` proves that Response treats reasoning markers as visible text.
...
- **`31-31`** and **`31-32`** put a non-ASCII name in an unfinished Qwen function header inside reasoning. The first ends at EOF; the second supplies a function closer, then a guided call. Both exercise UTF-8 boundaries in the header scanner, rather than Unicode confined to JSON arguments.
- **`31-33`** (`guided_json_native_parameter_body_inside_reasoning`) puts a complete native invocation inside guided reasoning, then sends the actual call as guided JSON. The native body is suppressed, surrounding reasoning survives, and the guided call dispatches. Splitting immediately after the native header must not expose its parameter body as reasoning. Every family uses the same Unicode and brace-like parameter value so grammar differences cannot hide a different stimulus.
- **`31-34`** and **`31-35`** start Qwen and Muse in Response with a visible reasoning-marker prefix, then place a JSON object or call array for Rome inside a native parameter body before the actual guided Paris call. The native body is suppressed; its braces cannot take ownership of the guided payload. Removing Response initialization changes the prefix to reasoning, so the state is observable.
- **`31-36`** starts Qwen with no prefilled state and interrupts an unfinished function header with a reasoning opener. The later function closer and JSON remain reasoning rather than borrowing a terminator across that opener. **`31-37`** starts Muse with no prefilled state and interrupts an unfinished quoted invoke name with a message-end marker; the name remains visible text and the following guided call dispatches.
- **`31-38`** begins DeepSeek V4 in Reasoning and repeats a reasoning opener inside an invoke body; it preserves the prior recovery output (`f">x` as reasoning, then the guided call). **`31-39`** begins Gemma in Reasoning and follows an incomplete `call:` prefix with a channel opener; the prefix and channel text remain reasoning even when the channel label arrives in the next chunk.
- **`31-40`** (`guided_json_reasoning_markers_inside_native_parameter`) puts the family's reasoning markers inside a native parameter value during guided reasoning. The native body stays suppressed; its quoted markers cannot close the outer thought or expose the remaining parameter markup. Surrounding reasoning survives, followed by the actual guided call.
...
- **`50-1`** (`prefilled_response_guided_pending_invoke_header`) follows a literal reasoning marker with an unfinished DeepSeek V4.1 invoke header and guided JSON. The visible prefix and call survive; the tool header does not become visible text between chunks.
- **`50-2`** (`prefilled_response_guided_closer_inside_invoke_quote`) covers the same Response recovery boundary with a Muse invoke closer inside the unfinished header's quoted name.
