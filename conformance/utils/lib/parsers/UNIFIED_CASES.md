...
- **`7-4`** (`arg_json_null`) All eight families must produce JSON `null`. The schema declares `city: string | null`, except GLM uses `city: null` because its union schema prefers string. Qwen3 and GLM supply bare parameter text `null`; other families use their native null syntax. This includes the former `qwen-1` case. Also covered in `TOOLCALLING.streamv1.7-4`.
- **`7-5`** (`arg_string_null`) All eight families must preserve the string `"null"` under `city: string`. Qwen3 and GLM use the same input text as `7-4`; other families use their native string syntax. Also covered in `TOOLCALLING.streamv1.7-5`.
