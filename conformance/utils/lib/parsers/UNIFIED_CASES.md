...
- DeepSeek V4.1 uses the ordered Unified contract for native DSML calls, reasoning interleaving, guided JSON, and prefilled states. The current corpus emits 77 of the 91 taxonomy cases for this family.
- Every taxonomy scenario declared for DeepSeek V4.1 is generated, except `40.a`, `40.c`, and `40.d`: each repeats an existing native Reasoning-to-call/text boundary with only different literal prose. The applicable cases include `30.m`; `31-1` through `31-25`, `31-27`, and `31-28`; the marker-discriminating Response row `50.d`; and `40.b` plus `41.a-b`.
- The 14 intentional not-applicable cases are `k3-1` through `k3-8`, which require Kimi K3 XTML syntax; `g4-1` through `g4-2`, which require Gemma 4 guided call-prefix syntax; `31-26`, which distinguishes Muse's tool-recipient header from its reasoning header; and the three redundant DeepSeek V4.1 rows above.
