...
- The 24 intentional not-applicable cases are `kimi_k2-1`; `kimi-1` through `kimi-8`; `gemma-1` through `gemma-3`; `muse-1` through `muse-3`; `qwen3-1` through `qwen3-3`; `deepseek_v4-1`; `35-3` through `35-4`; and the three redundant DeepSeek V4.1 rows above. These declarations include grammar-specific cases, narrowly scoped regressions, and literal-only duplicates. A scoped regression does not prove that omitted families need no equivalent test; that coverage assessment remains part of this follow-up.
