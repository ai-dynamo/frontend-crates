...
- **`31.x`** (`guided_json_unterminated_reasoning_then_wrapped_payload`) A thought whose closer never arrives, running straight into native tool markup wrapping the guided payload. `31.g` pins a wrapper around the payload OUTSIDE reasoning and `41.*` pins an unterminated thought on its own; neither asks what happens when the two meet, and that crossing is where both native families emitted the payload as REASONING and dispatched nothing. The client sees a plausible answer and never learns a call was lost. Contrast with `31.h`, where the same markup has PROSE behind it and is narration — what separates them is whether the guided payload follows, not which marker appeared.
- **`31-25`** (`guided_json_quoted_bare_header_in_answer`) and **`31-26`** (`guided_json_quoted_bare_tool_header_in_answer`) A bare header the model QUOTES inside its visible answer, after the turn has already been routed. The `to=…` words are prose and only the marker is structural. Promoting the quote split one answer into an answer plus a THOUGHT for a `to=self` quote, and silently DELETED the words for a tool recipient — two different corruptions of the same class. **muse_glimmer only, by design:** the scenario needs a header whose resolution depends on turn state, and a marker-pair family has no equivalent — its opener is a fixed string meaning the same thing wherever it appears, and a duplicate opener inside a span is already `41.c`. Scope is DECLARED, never inferred from a gap: these two wrap their per-family map in `OnlyFamilies({...})`, and a family missing from a PLAIN map still fails generation. Letting any absence read as "not applicable" would hide real missing coverage behind the same cell the table uses for a genuine structural gap, and a reviewer cannot tell those apart — `test_an_undeclared_missing_family_fails_generation` pins it.
- **`31-27`** (`guided_json_quoted_bare_header_after_payload`) and **`31-28`** (`guided_json_bare_tool_header_recovers_inside_a_thought`) The SCOPE pair. Turn position and open channel are independent axes, and `31-25`/`31-26` only cross one of them. `31-27` routes the turn with a PAYLOAD rather than a header, so a reader tracking only "has a header been seen" stays permissive and promotes a quote. `31-28` is the mirror: the same bare shape INSIDE an open thought is the missing-terminator recovery boundary, and a reader that closes its latch on the turn's first header demotes it and leaks `to=NAME` into the reasoning. Its golden keeps the separator SPACE in the thought (`"thinking "`), byte-for-byte what the native scan emits — a bare header absorbs that space when it opens a channel, which is right there and wrong at a recovery cut. What decides is scope, not whether a header has been seen — which is why one boolean gave the wrong answer in both directions at once.
- **`31-29`** (`gemma4_guided_json_visible_call_prose_before_reasoning`) and **`31-30`** (`gemma4_guided_json_malformed_call_prefix_before_reasoning`) are Gemma-only `call:` boundary cases. The first keeps ordinary visible `call:` prose before a thought and guided JSON. The second keeps the malformed but incomplete `call:get_weather` candidate as visible text because it lacks the `{` that starts a Gemma invoke body; the following thought and payload still route normally. They are intentionally `OnlyFamilies({gemma4})`: no other family gives this lexical prefix structural meaning, so cross-family rows would test nothing. Together they pin the boundary between plain prose and native-looking but incomplete Gemma syntax without losing the following reasoning or guided call.
