...
- **`5.d`** (`wrapped_saved_closer_partial_marker`) The family-native outer closer arrives while a nested argument value remains open, then EOF ends on a partial marker. The value is delimiter-terminated, so recover the call and preserve the trailing marker prefix as text. Every family receives its own grammar's form; implementations that cannot recover it render red.
- **`5.e`** (`bare_saved_closer`) The same real-closer recovery with the family-native outer opener removed. The offered tool name and outer closer bound the candidate; a bare call with no real closer remains unrecoverable. Every family receives its own grammar's form.
...
- **`6.b`** (`bare_parameterless_call`) A complete parameterless call with the family-native outer opener removed. The offered tool name and real outer closer make the malformed call bounded and recoverable. Every family receives its own grammar's form; implementations that reject it render red.
...
- **`31-31`** (`guided_json_native_envelope_after_prose`) A complete family-native tool envelope follows visible prose during a guided-JSON request. The envelope is stray specifically because GuidedJson already committed `hello ` as visible prose: it is neither permissible native syntax nor a new top-level guided payload. Strip it for every split instead of dispatching the native call or reinterpreting JSON nested in its parameter as a guided call. Every guided-capable family receives its own native envelope.
