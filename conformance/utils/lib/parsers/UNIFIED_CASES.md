...
- **`7-4`** (`arg_json_null`) The request tool schema declares `city` as `string | null`. GLM instead declares `city` as `null` because its parser prefers the string branch when given that union. All eight families must produce JSON `null`. Qwen3 and GLM use bare parameter text `null`; other families use native null syntax. This includes the former `qwen-1` case. Also covered in `TOOLCALLING.streamv1.7-4`.
- **`7-5`** (`arg_string_null`) The request tool schema declares `city` as non-nullable `string`. All eight families must preserve JSON string `"null"`. Qwen3 and GLM use the same bare parameter text `null` as `7-4`; other families use native string syntax. Also covered in `TOOLCALLING.streamv1.7-5`.
...
- DeepSeek V4.1 uses the ordered Unified contract for native DSML calls, reasoning interleaving, guided JSON, and prefilled states. The current corpus emits 83 of the 95 taxonomy cases for this family.
...
- The 12 omitted cases are `kimi-1` through `kimi-8`, which require Kimi K3 XTML syntax; `gemma-1` through `gemma-2`, which require Gemma 4 guided call-prefix syntax; `glm5-1`, which requires GLM's argument-marker grammar; `muse-1`, whose non-Muse variant duplicates `35-1`. The `muse-1` duplicate does not imply that quoted or malformed model output cannot occur.
