...
Groups 1–12 vary the model OUTPUT. Groups 30+ vary the request: what the serving layer passed to `UnifiedParser::initialize_with_output_mode(starting_state, tool_output_mode)` before any output arrived. The pairing is `X0` happy / `X1` malformed, and a new mode takes the next ten.
