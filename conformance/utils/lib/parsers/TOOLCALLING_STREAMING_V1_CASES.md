...
- **`TOOLCALLING.streamv1.7-4`** A nullable string schema converts bare parameter text `null` to JSON `null` in Qwen3 Coder, MiniMax M2, and MiniMax M3; each character arrives in a separate chunk. GLM prefers string in this union. Other grammars use quotes, string delimiters, or explicit type attributes instead of this schema-dependent ambiguity. Exercises `TOOLCALLING.xml.2` schema-aware coercion.
- **`TOOLCALLING.streamv1.7-5`** A non-nullable string schema preserves bare parameter text `null` as the string `"null"` in Qwen3 Coder, GLM, MiniMax M2, and MiniMax M3; the value is split inside `null`. Exercises `TOOLCALLING.xml.2` schema-aware coercion.
