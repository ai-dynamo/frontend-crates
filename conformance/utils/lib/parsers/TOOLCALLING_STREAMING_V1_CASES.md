...
- **`TOOLCALLING.streamv1.7-4`** The request tool schema declares `city` as `string | null`. GLM instead declares `city` as `null` because its parser prefers the string branch when given that union. Qwen3 Coder, MiniMax M2, and MiniMax M3 convert bare parameter text `null` to JSON `null`, with each character arriving in a separate chunk. GLM prefers string in this union. Other grammars use native null syntax. Exercises `TOOLCALLING.xml.2` schema-aware coercion.
- **`TOOLCALLING.streamv1.7-5`** The request tool schema declares `city` as non-nullable `string`. Qwen3 Coder, GLM, MiniMax M2, and MiniMax M3 preserve bare parameter text `null` as JSON string `"null"`; the value is split inside `null`. Other grammars use native string syntax. Exercises `TOOLCALLING.xml.2` schema-aware coercion.
