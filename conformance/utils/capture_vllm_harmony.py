...
an openai_harmony StreamableParser (token-native, like Dynamo's harmony parser),
...
# <|start|>assistant — exactly the normalization Dynamo's harmony parser does — so
        ...
        for chunk in case.get("chunks", []):
            ...
            for tid in ids:
                ...
                try:
                    ...
                    # Mirror Dynamo's parser, which breaks and keeps what it emitted.
                    broken = True
                    break
                ts = [
                    TokenState(
