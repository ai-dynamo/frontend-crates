...
fn toolcalling_stream_parity() {
    ...
    for path in &files {
        ...
        // Both harmony families have a Dynamo streaming parser: `harmony`
        // (token-id path) and `harmony_text` (text path). All other families are
        // dynamo-unavailable (TODO) and carry no expected.dynamo to assert.
    ...
    eprintln!(
        "harmony streaming parity: {}/{} cases passed ({skipped} dynamo-unavailable)",
