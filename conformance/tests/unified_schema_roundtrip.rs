        ...
        for (id, case) in file.cases {
            for event in case.golden {
                ...
                for (key, value) in arguments.as_object().into_iter().flatten() {
                    ...
                    // Kimi K3 explicitly exercises typed native argument values.
                    // Shared XML-family cases intentionally keep `g.y` as a string.
                    if file.family == "kimi_k3"
                        && id == "UNIFIED.kimi_k3_malformed_call_then_valid.kimi_k3"
                        && name == "g"
                        && key == "y"
                    {
                        compatible = value
