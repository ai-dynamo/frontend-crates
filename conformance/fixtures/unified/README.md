...
Retained historical archives stay byte-for-byte. Manifest `inactive_shards` entries retain superseded shared corpora and the mislabeled `0.6.0` capture as hash-pinned evidence, excluded from active extraction and rendering. A capture without request bindings cannot prove that its output belongs to the displayed request, so it is removed from the active history rather than rendered as an empty result. A history whose scoreable outcomes are exactly covered by a later capture is also removed with its manifest shard. The disposition names an exact archive and hash, not every capture of that version.
